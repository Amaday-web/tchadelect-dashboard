...
if uploaded_file is not None:
    ...
    # Prédiction consommation mois suivant
    df_mensuel = df.groupby("mois")["consommation_kwh"].sum().reset_index()
    df_mensuel["mois_num"] = np.arange(len(df_mensuel))
    model = LinearRegression()
    model.fit(df_mensuel[["mois_num"]], df_mensuel["consommation_kwh"])
